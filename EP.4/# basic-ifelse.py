...
if money >= 500:
	...
	print('คุณผู้หญิงทานร้าน {} ดีไหม? ราคาเริ่มต้น {} บาท'.format(select['name'],select['price']))
elif money >= 200:
	select = random.choice(restaurant['medium'])
	print('คุณพี่ทานร้าน {} ดีไหม? ราคาเริ่มต้น {} บาท'.format(select['name'],select['price']))
else:
	select = random.choice(restaurant['low'])
	print('พี่ทานร้าน {} ดีไหม? ราคาเริ่มต้น {} บาท'.format(select['name'],select['price']))
